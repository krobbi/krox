...
static Entry* findEntry(Entry* entries, int capacity, ObjString* key) {
	uint32_t index = key->hash & (capacity - 1); /* Faster modulo. */
	...
	for (;;) {
		...
		index = (index + 1) & (capacity - 1); /* Faster modulo. */
	}
}
...
ObjString* tableFindString(Table* table, const char* chars, int length, uint32_t hash) {
	...
	uint32_t index = hash & (table->capacity - 1); /* Faster modulo. */
	...
	for (;;) {
		...
		index = (index + 1) & (table->capacity - 1); /* Faster modulo. */
	}
}
